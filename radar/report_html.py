...
function openLightbox(src,air){
 ...
 const airBtns=air?`<button class="chip on" id="lb_regen">🔄 AI ilə yenidən</button><label class="chip on" style="cursor:pointer;margin:0">📤 Öz şəklini yüklə<input type="file" id="lb_up" accept="image/*" style="display:none"></label>`:'';
 ov.innerHTML=`<img id="lb_img" src="${base+'?v='+Date.now()}" style="max-width:92vw;max-height:76vh;object-fit:contain;border-radius:12px;box-shadow:0 12px 48px rgba(0,0,0,.6)">
   <div class="controls" style="margin:0">${airBtns}<a class="chip" href="${base}" target="_blank" style="text-decoration:none">↗ Tam ölçü</a><button class="chip" id="lb_close">✕ Bağla</button></div>
 ...
 if(!air)return;
 const p=air.split(':'),did=+p[0],idx=+p[1];
 const applyNew=()=>{const bust='?v='+Date.now();document.getElementById('lb_img').src=base+bust;const th=document.querySelector('.dimg[data-air="'+air+'"]');if(th)th.src=(th.dataset.full.split('?')[0])+bust;};
 document.getElementById('lb_regen').onclick=async()=>{const btn=document.getElementById('lb_regen');btn.disabled=true;btn.textContent='⏳ Yaradılır (~20s)…';document.getElementById('lb_msg').textContent='AI real fotodan yeni brendlənmiş versiya…';
   ...
   if(r.ok){applyNew();document.getElementById('lb_msg').textContent='✅ AI ilə əvəz olundu';}else document.getElementById('lb_msg').textContent='⚠️ '+((r.error||JSON.stringify(r))+'').slice(0,140);
   btn.disabled=false;btn.textContent='🔄 AI ilə yenidən';};
 document.getElementById('lb_up').onchange=async(e)=>{const f=e.target.files[0];if(!f)return;document.getElementById('lb_msg').textContent='📤 Yüklənir: '+f.name+' …';
   ...
   if(r.ok){applyNew();document.getElementById('lb_msg').textContent='✅ Öz şəklin əlavə olundu';}else document.getElementById('lb_msg').textContent='⚠️ '+((r.error||JSON.stringify(r))+'').slice(0,140);
   e.target.value='';};
}
function bindDrafts(){const g=id=>document.getElementById(id);const J={'Content-Type':'application/json'};
 document.querySelectorAll('[data-dv]').forEach(b=>b.onclick=async()=>{const id=b.dataset.dv,box=g('dv_'+id);if(box.style.display==='block'){box.style.display='none';return;}const d=await api('/api/draft/get?id='+id);box.style.display='block';
  ...
  const orig=`<div class="grid2"><div><div class="small" style="font-weight:700">📄 Orijinal başlıq</div><input value="${esc(d.title||'')}" id="dt_${id}" style="width:100%"><div class="small" style="font-weight:700;margin-top:6px">Orijinal təsvir</div><textarea id="db_${id}" style="width:100%;min-height:80px">${esc(d.body||'')}</textarea><div style="margin-top:6px"><span class="small" style="font-weight:700">Qiymət </span><input value="${d.price||''}" id="dp_${id}" style="width:110px"> <button class="chip" data-dsave="${id}">💾 Saxla</button></div><div class="small muted" style="margin-top:6px">Atributlar: ${(d.properties.collection||[]).map(c=>esc(c.text||c.value)).join(', ')||'—'}</div></div>
   <div><div class="small" style="font-weight:700">📷 Orijinal şəkillər (${d.n_photos})</div><div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px">${(d.photos||[]).map(u=>`<img class="dimg" data-full="${u}" src="${u}" style="width:82px;height:82px;object-fit:cover;border-radius:8px;border:1px solid var(--line);cursor:zoom-in">`).join('')||'<span class="muted">yox</span>'}</div></div></div>`;
  const aiSection=`<div class="panel" style="margin:10px 0 0;background:var(--acc-soft)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px"><b>🤖 PCTECH brend-versiyası</b><button class="chip on" data-aiadapt="${id}">${aiOn?'🔄 Yenidən brendlə':'✨ PCTECH-ə çevir (mətn + şəkli brendlə)'}</button></div>
    ${aiOn?`<div class="grid2" style="margin-top:8px"><div><div class="small" style="font-weight:700;color:var(--acc2)">✨ PCTECH başlıq</div><div style="font-weight:700">${esc(d.adapted_title||'—')}</div><div class="small" style="font-weight:700;margin-top:6px;color:var(--acc2)">✨ PCTECH təsvir</div><div class="small" style="white-space:pre-wrap;max-height:140px;overflow:auto">${esc(d.adapted_body||'—')}</div></div>
     <div><div class="small" style="font-weight:700;color:var(--acc2);display:flex;align-items:center;gap:8px;flex-wrap:wrap">✨ Brendlənmiş şəkillər (${d.n_ai_photos||0}) <span class="muted" style="font-weight:400">— kliklə: zoom / əvəz</span><label class="chip" style="cursor:pointer;font-size:10px;padding:2px 8px;margin:0">📤 Öz şəklini əlavə et<input type="file" id="aiadd_${id}" accept="image/*" style="display:none"></label></div><div id="aiimgs_${id}" style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px">${(d.ai_photos||[]).map((u,i)=>`<img class="dimg" data-full="${u}" data-air="${id}:${i}" src="${u}" style="width:82px;height:82px;object-fit:cover;border-radius:8px;border:2px solid var(--acc2);cursor:zoom-in">`).join('')||'<span class="muted">şəkil yox</span>'}</div></div></div>
     <div class="small muted" style="margin-top:6px">✅ «Təsdiqlə → tap.az» AI-versiyanı (PCTECH mətn + brendlənmiş şəkil) göndərəcək.</div>`:(BACKEND.ai_key?'<div class="small muted" style="margin-top:6px">✨ düyməsi: mətn PCTECH-ə çevrilir + real tap.az şəkilləri brendlənir (məhsul eyni qalır, yalnız fon/təqdimat).</div>':'<div class="small" style="margin-top:6px;color:var(--bad)">⚠️ OpenAI açarı yoxdur — <b>⚙️ Tənzimləmələr</b> səhifəsindən əlavə et.</div>')}
    <div id="aimsg_${id}" class="small" style="margin-top:6px"></div></div>`;
  box.innerHTML=orig+aiSection;
  box.querySelectorAll('.dimg').forEach(img=>img.onclick=()=>openLightbox(img.dataset.full,img.dataset.air||''));
  if(g('aiadd_'+id))g('aiadd_'+id).onchange=async(e)=>{const f=e.target.files[0];if(!f)return;g('aimsg_'+id).textContent='📤 Yüklənir: '+f.name+' …';const b64=await new Promise(res=>{const rd=new FileReader();rd.onload=()=>res(rd.result);rd.readAsDataURL(f);});const r=await api('/api/draft/replace-photo',{method:'POST',headers:J,body:JSON.stringify({id:+id,index:-1,b64})});if(r.ok){const cont=g('aiimgs_'+id),ph=cont.querySelector('span.muted');if(ph)ph.remove();const u='/drafts_media/'+id+'/ai_'+r.index+'.jpg',air=id+':'+r.index;const im=document.createElement('img');im.className='dimg';im.dataset.full=u;im.dataset.air=air;im.src=u+'?v='+Date.now();im.style.cssText='width:82px;height:82px;object-fit:cover;border-radius:8px;border:2px solid var(--acc2);cursor:zoom-in';im.onclick=()=>openLightbox(u,air);cont.appendChild(im);g('aimsg_'+id).textContent='✅ Öz şəklin əlavə olundu (#'+r.index+')';}else g('aimsg_'+id).textContent='⚠️ '+((r.error||JSON.stringify(r))+'').slice(0,140);e.target.value='';};
  ...
  box.querySelector('[data-aiadapt]').onclick=async(ev)=>{ev.target.textContent='⏳ Brendlənir (mətn + şəkillər, ~1 dəq)…';ev.target.disabled=true;const r=await api('/api/draft/ai-adapt',{method:'POST',headers:J,body:JSON.stringify({id:+id})});if(r.ok){g('aimsg_'+id).innerHTML='✅ Mətn hazır'+(r.n_ai_photos?` + ${r.n_ai_photos} brendlənmiş şəkil`:'')+(r.image_error?(' ⚠️ şəkil: '+esc(r.image_error.slice(0,80))):'');loadDrafts();}else g('aimsg_'+id).innerHTML='⚠️ '+esc((r.error||JSON.stringify(r)).slice(0,220));};});
