...
function openLightbox(src,air){
 ...
 ov.innerHTML=`<img id="lb_img" src="${base+'?v='+Date.now()}" style="max-width:92vw;max-height:78vh;object-fit:contain;border-radius:12px;box-shadow:0 12px 48px rgba(0,0,0,.6)">
   <div class="controls" style="margin:0">${air?`<button class="chip on" id="lb_regen">🔄 Yenisi ilə əvəz et</button>`:''}<a class="chip" href="${base}" target="_blank" style="text-decoration:none">↗ Tam ölçü</a><button class="chip" id="lb_close">✕ Bağla</button></div>
 ...
 if(air){const p=air.split(':'),did=+p[0],idx=+p[1];
  document.getElementById('lb_regen').onclick=async()=>{const btn=document.getElementById('lb_regen');btn.disabled=true;btn.textContent='⏳ Yeni şəkil yaradılır (~20s)…';document.getElementById('lb_msg').textContent='Real fotodan yeni brendlənmiş versiya…';
   ...
   if(r.ok){const bust='?v='+Date.now();document.getElementById('lb_img').src=base+bust;const th=document.querySelector('.dimg[data-air="'+air+'"]');if(th)th.src=(th.dataset.full.split('?')[0])+bust;document.getElementById('lb_msg').textContent='✅ Əvəz olundu';btn.disabled=false;btn.textContent='🔄 Yenisi ilə əvəz et';}
   else{document.getElementById('lb_msg').textContent='⚠️ '+((r.error||JSON.stringify(r))+'').slice(0,140);btn.disabled=false;btn.textContent='🔄 Yenisi ilə əvəz et';}};}
}
 ...
 document.querySelectorAll('[data-dv]').forEach(b=>b.onclick=async()=>{const id=b.dataset.dv,box=g('dv_'+id);if(box.style.display==='block'){box.style.display='none';return;}const d=await api('/api/draft/get?id='+id);box.style.display='block';
  ...
  const aiSection=`<div class="panel" style="margin:10px 0 0;background:var(--acc-soft)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px"><b>🤖 PCTECH brend-versiyası</b><button class="chip on" data-aiadapt="${id}">${aiOn?'🔄 Yenidən brendlə':'✨ PCTECH-ə çevir (mətn + şəkli brendlə)'}</button></div>
    ${aiOn?`<div class="grid2" style="margin-top:8px"><div><div class="small" style="font-weight:700;color:var(--acc2)">✨ PCTECH başlıq</div><div style="font-weight:700">${esc(d.adapted_title||'—')}</div><div class="small" style="font-weight:700;margin-top:6px;color:var(--acc2)">✨ PCTECH təsvir</div><div class="small" style="white-space:pre-wrap;max-height:140px;overflow:auto">${esc(d.adapted_body||'—')}</div></div>
     <div><div class="small" style="font-weight:700;color:var(--acc2)">✨ Brendlənmiş şəkillər (${d.n_ai_photos||0}) <span class="muted" style="font-weight:400">— kliklə: zoom + əvəz et</span></div><div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px">${(d.ai_photos||[]).map((u,i)=>`<img class="dimg" data-full="${u}" data-air="${id}:${i}" src="${u}" style="width:82px;height:82px;object-fit:cover;border-radius:8px;border:2px solid var(--acc2);cursor:zoom-in">`).join('')||'<span class="muted">şəkil yox</span>'}</div></div></div>
