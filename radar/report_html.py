 ...
 return _sysHead()+`<div class="panel"><h2>🎨 Kart dizaynı <span class="small">— dəyiş → canlı önizlə → saxla</span></h2>
   <div class="grid2" style="gap:20px">
    ...
    <div>
     ...
     <div class="small muted" style="margin-top:6px">Nümunə məhsul göstərilir. Real kart hər draftın «🟩 PCTECH kart» düyməsi ilə yaranır.</div>
    </div>
   </div></div>`;
}
...
async function openStorePreview(id){let ov=document.getElementById('stprev');
 ...
 if(d.draft_id&&(d.n_ai_photos||0)>0)renderPvAi(d.draft_id,d.n_ai_photos);}
function renderPvAi(did,n){const el=document.getElementById('stpv_ai');if(!el)return;const JH={'Content-Type':'application/json'};
 let h='<div style="font-weight:700;margin:16px 0 6px">✨ PCTECH şəkilləri <span class="muted small">— 1-ci çərçivəli, digərləri ağ fon · hər birini dəyiş</span></div><div style="display:flex;gap:10px;flex-wrap:wrap">';
 for(let i=0;i<n;i++)h+=`<div style="width:148px"><img id="pvai_${i}" src="/drafts_media/${did}/ai_${i}.jpg?v=${Date.now()}" style="width:148px;height:148px;object-fit:contain;background:#fff;border:1px solid var(--line);border-radius:10px"><div class="controls" style="gap:3px;margin-top:4px;justify-content:center"><button class="chip" data-sty="card" data-i="${i}" title="Çərçivəli kart" style="padding:2px 7px">🟩</button><button class="chip" data-sty="white" data-i="${i}" title="Ağ fon" style="padding:2px 7px">⬜</button><button class="chip" data-sty="original" data-i="${i}" title="Orijinal" style="padding:2px 7px">📷</button><label class="chip" style="padding:2px 7px;cursor:pointer;margin:0" title="Öz şəklin">📤<input type="file" accept="image/*" data-up="${i}" style="display:none"></label></div></div>`;
 el.innerHTML=h+'</div>';
 const refresh=i=>{document.getElementById('pvai_'+i).src='/drafts_media/'+did+'/ai_'+i+'.jpg?v='+Date.now();};
 el.querySelectorAll('[data-sty]').forEach(b=>b.onclick=async()=>{const i=b.dataset.i,ic=b.textContent;b.textContent='⏳';const r=await api('/api/draft/make-card',{method:'POST',headers:JH,body:JSON.stringify({id:did,index:+i,style:b.dataset.sty})});b.textContent=ic;if(r.ok)refresh(i);else pvMsg('⚠️ '+esc((r.error||'').slice(0,80)));});
 el.querySelectorAll('[data-up]').forEach(inp=>inp.onchange=async(e)=>{const f=e.target.files[0];if(!f)return;const i=inp.dataset.up;const b64=await new Promise(r=>{const rd=new FileReader();rd.onload=()=>r(rd.result);rd.readAsDataURL(f);});const r=await api('/api/draft/replace-photo',{method:'POST',headers:JH,body:JSON.stringify({id:did,index:+i,b64,raw:true})});if(r.ok)refresh(i);e.target.value='';});}
...
function renderPvActions(d,id){const el=document.getElementById('stpv_act');if(!el)return;const JH={'Content-Type':'application/json'};const g=x=>document.getElementById(x);
 ...
 else if(d.draft_id)h=`<button class="chip on" id="stpv_card">${(d.n_ai_photos>0)?'🔄 Şəkilləri yenidən yarat':'🟩 PCTECH kart dəsti (3)'}</button><button class="chip on" id="stpv_appr" style="background:var(--good)">✅ Təsdiqlə → tap.az</button>`;
 ...
 if(g('stpv_card'))g('stpv_card').onclick=async()=>{const b=g('stpv_card');b.textContent='⏳ Şəkil dəsti (~1 dəq)…';b.disabled=true;pvMsg('🟩 PCTECH şəkil dəsti yaradılır — 1 çərçivəli + qalan ağ fon…');const r=await api('/api/draft/make-set',{method:'POST',headers:JH,body:JSON.stringify({id:d.draft_id,n:3})});if(r.ok){d.n_ai_photos=r.n;pvMsg('✅ '+r.n+' şəkil hazır — hər birini dəyişə bilərsən (🟩 çərçivə / ⬜ ağ / 📷 orijinal / 📤 öz), sonra Təsdiqlə.');renderPvAi(d.draft_id,r.n);loadStoreProducts(true);}else pvMsg('⚠️ '+esc((r.error||JSON.stringify(r)).slice(0,140)));renderPvActions(d,id);};
...
function openLightbox(src,ctx){
 ...
 const btns=act?`<button class="chip on" id="lb_card">🟩 PCTECH kart</button>${ctx.isOrig?'':`<button class="chip" id="lb_regen">🤖 AI təmizlə</button>`}<label class="chip" style="cursor:pointer;margin:0">📤 Öz şəklini yüklə<input type="file" id="lb_up" accept="image/*" style="display:none"></label>`:'';
 ...
 document.getElementById('lb_card').onclick=async()=>{const b=document.getElementById('lb_card');b.disabled=true;b.textContent='⏳ Kart (~30s)…';msg('🟩 PCTECH sabit-dizayn kartı yaradılır (məhsul təmizlənir + montaj)…');
   ...
   if(r.ok){showResult(r.index);msg('✅ PCTECH kart hazırdır');}else msg('⚠️ '+((r.error||JSON.stringify(r))+'').slice(0,150));b.disabled=false;b.textContent='🟩 PCTECH kart';};
...
async function renderDraftDetail(id,box){const g=x=>document.getElementById(x);const J={'Content-Type':'application/json'};
  ...
  const aiSection=`<div class="panel" style="margin:10px 0 0;background:var(--acc-soft)"><div style="display:flex;justify-content:space-between;align-items:center;flex-wrap:wrap;gap:6px"><b>🟩 PCTECH brend-versiyası</b><div class="controls" style="margin:0"><button class="chip on" data-makeset="${id}">🟩 Kart dəsti (3)</button><button class="chip on" data-aiadapt="${id}">${aiOn?'🔄 Mətni yenilə':'✨ Mətni çevir'}</button></div></div>
    ${aiOn?`<div class="grid2" style="margin-top:8px"><div><div class="small" style="font-weight:700;color:var(--acc2)">✨ PCTECH başlıq</div><div style="font-weight:700">${esc(d.adapted_title||'—')}</div><div class="small" style="font-weight:700;margin-top:6px;color:var(--acc2)">✨ PCTECH təsvir</div><div class="small" style="white-space:pre-wrap;max-height:140px;overflow:auto">${esc(d.adapted_body||'—')}</div></div>
     ...
     <div class="small muted" style="margin-top:6px">✅ «Təsdiqlə → tap.az» — hər şəkil üçün brendli/kart varsa onu, yoxsa orijinalı göndərir (hər şəkli ayrı seçirsən).</div>`:(BACKEND.ai_key?'<div class="small muted" style="margin-top:6px">✨ mətni PCTECH-a çevirir. Sonra hər şəkli kliklə: <b>🟩 PCTECH kart</b>, 📤 öz şəklin, və ya orijinalı saxla.</div>':'<div class="small" style="margin-top:6px;color:var(--bad)">⚠️ OpenAI açarı yoxdur — <b>⚙️ Tənzimləmələr</b> səhifəsindən əlavə et.</div>')}
  ...
  box.querySelector('[data-aiadapt]').onclick=async(ev)=>{ev.target.textContent='⏳ Mətn PCTECH-a çevrilir…';ev.target.disabled=true;const r=await api('/api/draft/ai-adapt',{method:'POST',headers:J,body:JSON.stringify({id:+id})});if(r.ok){refresh();}else g('aimsg_'+id).innerHTML='⚠️ '+esc((r.error||JSON.stringify(r)).slice(0,220));};
  const ms=box.querySelector('[data-makeset]');if(ms)ms.onclick=async()=>{ms.textContent='⏳ Şəkil dəsti (~1 dəq)…';ms.disabled=true;g('aimsg_'+id).textContent='🟩 PCTECH şəkil dəsti yaradılır — 1 çərçivəli + qalan ağ fon…';const r=await api('/api/draft/make-set',{method:'POST',headers:J,body:JSON.stringify({id:+id,n:3})});if(r.ok){g('aimsg_'+id).textContent='✅ '+r.n+' şəkil hazır';refresh();}else{g('aimsg_'+id).textContent='⚠️ '+((r.error||JSON.stringify(r))+'').slice(0,140);ms.disabled=false;ms.textContent='🟩 Kart dəsti (3)';}};
}
